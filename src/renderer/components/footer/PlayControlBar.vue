<template>
  <div class="play-control-bar">
    <div class="control">
      <img src="~@/assets/img/icon-prev.svg" class="prev" @click="onPrev">
      <span class="s-p play" @click="ControlPlay"
            :class="{ 'stop': $store.state.player.playing }"></span>
      <img src="~@/assets/img/icon-next.svg" class="next" @click="onNext">
    </div>
    <div class="progress">
    ...
    <div class="other">
      <span><icon
        class="icon-playlist"
        size="18px"
        :normal="require('@/assets/img/list.svg')"
...
<script>
  ...
  export default {
    name: "PlayControlBar",
    components: {IconSwitch, Volume, ProgressBar, Icon},
    data() {
      return {
        ctime: 0,
        duration: 0,
    ...
    created() {
      ...
      this.$bus.$on(BusTypes.CLEAR_PLAYLIST, () => {
        this.$audioer.currentTime = 0;
        this.$audioer.pause()
        this.$store.commit(StoreTypes.CLEAR_PLAYLIST)
      })
    ...
    methods: {
      ...
      play(id) {
        this.pause()
        this.$audioer.src = `http://localhost:11015/songstream?id=${id}&sId=${this.$store.state.SID}`
      ...
      togglePlayList(){
        this.$store.commit(StoreTypes.TOGGLE_PLAY_LIST)
      }
    }
  }
</script>
